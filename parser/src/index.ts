import { CreateMapJSON, CreateWeaponJSON, MAP_DEF, SearchManifest, WEAPON_DEF } from "./Search"
import Setup from "./Setup"
import ironBannerWeaponNames from "./ironBannerWeapons"
import { ITEM_DEFS } from "./Querry";
...
const MakeWeek = async (db:any,week:string, keyRow:any, weeks:string[])=>{
  var totalCells
  ...
  if(items.length>2){
    ...
    const ironBannerData = (TRIALS_GUN.name ? null :{
      name:"Iron Banner"
    })
...
const run = async()=>{
  ...
  fs.writeFileSync(exportPath+fileName, JSON.stringify(fileJSON))
...
run()
// MakeGun("spare rations").then(console.log)